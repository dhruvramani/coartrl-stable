            ...
            for step in range(total_timesteps):
                ...
                if (self.num_timesteps < self.learning_starts
                    ...
                    if(not self.config.is_coart):
                        action = self.policy_tf.step(obs, deterministic=False).flatten() # obs[None]
                    elif(self.config.stitch_naive):
                        action = prim_pi.step(obs, deterministic=False).flatten()
                    else :
                        action = self.bridge_policy.step(obs, deterministic=False).flatten()
