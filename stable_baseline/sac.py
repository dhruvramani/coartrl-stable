...
from stable_baselines.common import OffPolicyRLModel, SetVerbosity, TensorboardWriter
from stable_baselines.common.vec_env import VecEnv
#from stable_baseline.sac.policies import SACPolicy
from stable_baselines.deepq.replay_buffer import ReplayBuffer
from stable_baselines.ppo2.ppo2 import safe_mean, get_schedule_fn

from baselines import logger
import baselines.common.tf_util as tf_util
from stable_baseline.sac_policy import SACPolicy

from util import ObsNormalizer
...
# TODO : Put observation normalization for SAC
#        Might have to incroporate primitive policy w/ SAC Policy
...
class SAC(OffPolicyRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=3e-4, buffer_size=50000,
                 ...
                 primitives=None, bridge_policy=None, config=None):
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            if sys.platform == 'darwin':
                n_cpu //= 2
            self.sess = tf_util.get_session() #tf_util.make_session(num_cpu=n_cpu, graph=self.graph)
            ...
            with tf.variable_scope("input", reuse=False):
                ...
                self.ob_normalizer = ObsNormalizer(self.env, "JacoToss-v1", self.config, num_policies=2, policy_const=self.policy, 
                    policy_args=[self.sess, self.observation_space, self.action_space, self.policy_kwargs])
                
                self.policy_tf = self.ob_normalizer.policies[0]
                self.target_policy = self.ob_normalizer.policies[1]
    ...
    def learn(self, total_timesteps, callback=None, seed=None,
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            # NOTE : @dhruvramani
            curr_prim = 0
            prim_pi = self.primitives[curr_prim]

            obs_l = self.ob_normalizer.normalize_obv(self.env, obs)
            ...
            for step in range(total_timesteps):
                ...
                # NOTE : @dhruvramani
                if(self.config.is_coart and self.config.stitch_naive and 
                    curr_prim == 0 and prim_pi.is_terminate(obs, init=True, env=self.env)):
                    curr_prim = 1
                    prim_pi = self.primitives[curr_prim]
                ...
                if (self.num_timesteps < self.learning_starts
                    ...
                    # NOTE : @dhruvramani
                    if(not self.config.is_coart):
                        action = self.policy_tf.step(obs_l, deterministic=False).flatten() # obs[None]
                    elif(self.config.stitch_naive):
                        action, _, _, _ = prim_pi.step(obs, deterministic=False)
                    else :
                        action, _, _, _ = self.bridge_policy.step(obs, deterministic=False)
                    ...
                    rescaled_action = action * np.abs(self.action_space.low[0])
                ...
                # NOTE : @dhruvramani
                new_obs_l = self.ob_normalizer.normalize_obv(self.env, new_obs)
