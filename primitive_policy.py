...
class PrimitivePolicy(ActorCriticPolicy):
    def __init__(self, name, env, ob_env_name, n_env, config=None):
        # configs
        self._config = config
        ...
        super(PrimitivePolicy, self).__init__(None, self._ob_space, self._ac_space, n_env, 1, None, reuse=False, scale=False, obs_phs=self.obs_phs)
    ...
    def _build(self):
        ...
        self._policy = pdparam
        self._proba_distribution = self.pd
        self._value_fn = self.vpred
    ...
    def step(self, obs, state=None, mask=None, deterministic=False):
        ...
        return ac[0], vpred[0], state, None

    def value(self, obs, state=None, mask=None, deterministic=False):
        stochastic = not deterministic
        if self.hard_coded:
            return 0
        ob_list = self.get_ob_list(obs)
